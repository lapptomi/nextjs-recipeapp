...
type CategoryValue = "all" | "breakfast" | "lunch" | "dinner" | "dessert" | "snack";
...
export default function RecipeFilters({ category = "all" }: Props) {
  const navigate = useNavigate();
  const [selected, setSelected] = useState<CategoryValue>(
    CATEGORY_OPTIONS.some((c) => c.value === category) ? (category as CategoryValue) : "all"
  );
