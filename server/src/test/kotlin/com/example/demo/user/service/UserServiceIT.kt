...
class UserServiceIT {
    ...
    fun `should not create user with duplicate email`() {
        ...
        val userWithDuplicateEmail =
            CreateUserRequestDTO(username = "user4", email = users[0].email, password = "password4")
    ...
    fun `should not create user with too long username`() {
        ...
        assertEquals(usersSize, userService.getUsers().size)
        assertEquals(
            "could not execute statement [Value too long for column " +
                "\"USERNAME CHARACTER VARYING(30)\": \"'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' (31)\"; SQL statement:\n" +
                "insert into users (created_at,email,password,username,id) " +
                "values (?,?,?,?,?) [22001-224]] [insert into users (created_at,email,password,username,id) " +
                "values (?,?,?,?,?)]; SQL [insert into users (created_at,email,password,username,id) values (?,?,?,?,?)]",
            error.message,
        )
    }
    ...
    fun `should not create user with too long email`() {
        ...
        assertEquals(usersSize, userService.getUsers().size)
        assertEquals(
            "could not execute statement [Value too long for column \"EMAIL CHARACTER VARYING(100)\": " +
                "\"'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa... (101)\"; SQL statement:\n" +
                "insert into users (created_at,email,password,username,id) " +
                "values (?,?,?,?,?) [22001-224]] [insert into users (created_at,email,password,username,id)" +
                " values (?,?,?,?,?)]; SQL [insert into users (created_at,email,password,username,id) values (?,?,?,?,?)]",
            error.message,
